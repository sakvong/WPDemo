...
function shortcode_button($atts, $content = null) {
	...
	return '<a class="newave-button ' . $atts['size'] . ' ' . $shape . ' ' . $outline . ' ' . $move . ' ' . $external . ' '. $atts['color'] . '" href="' . $atts['link'] . '" target="' . $atts['target'] . '">' .do_shortcode($content). '</a>';
}
...
// Team Modern
//////////////////////////////////////////////////////////////////
add_shortcode('team_modern', 'shortcode_team_modern');
function shortcode_team_modern($atts, $content = null) {

    $out = '';
    $out .= '<ul class="our-team-modern">';
    $out .= do_shortcode( $content );
    $out .= '</ul>';
...
add_shortcode('team_member_modern', 'shortcode_team_member_modern');
function shortcode_team_member_modern($atts, $content = null) {
    ...
    $out .= '<li class="team-person-modern" style="background-image: url(\''. $atts['picture'] .'\')">';
	$out .= '<div class="team-profile-modern">';
    $out .= '<div class="outer">';
    $out .= '<div class="inner">';
    $out .= '<h5>'.$atts['name'].'</h5>';
	$out .= $atts['title'];
...
function shortcode_recent_posts($atts, $content = null) {
    ...
    while ( $posts_query->have_posts()) {
        ...
        $str .= '<p class="blog-meta">' . __( 'Posted by', 'newave_theme' ) . '<a href="' . $author_url . '">' . get_the_author() . '</a> | ' . $str_categories . ' | <a href="' . get_comments_link() . '"> ' . get_comments_number() . __( 'comment(s)', 'newave_theme' ) . ' </a></p>';
        $str .= '</div>';
    }
...
function shortcode_posts_carousel($atts, $content = null) {
	...
	$str .= '<div class="news-carousel-container">';
	$str .= '<div id="news-carousel">';
    ...
    while ( $posts_query->have_posts()) {
        ...
        if( strlen( $post_content ) > $atts['excerpt'] ){

            $post_content = substr( $post_content, 0, $atts['excerpt'] );

            $post_content .= "[...]";
        }

		$str .= '<div class="news-carousel-item">';
		$str .= '<a href="' . get_permalink() . '"><h4 class="news-title">' . $post_title . '</h4></a>';
		$str .= '<p class="news-content">' . $post_content . '</p>';
		
		$author_id = get_the_author_meta("ID");
        $author_url = get_author_posts_url( $author_id );
		$str .= '<p class="news-author"><a href="' . $author_url . '">' . get_avatar( $author_id ) . __( 'by', 'newave_theme' ) . ' ' . get_the_author() . '</a></p>';
		$str .= '</div>';
	...
	$str .= '<p><a class="prev" id="cnews_prev" href="#"><i class="fa fa-angle-left"></i></a><a class="next" id="cnews_next" href="#"><i class="fa fa-angle-right"></i></a></div>';
	$str .= '<div id="news-pag" class="pagination"></div>';
	$str .= '</div>';
...
function shortcode_parallax_twitter($atts, $content = null) {
    ...
    if( function_exists('getTweets') ){
        ...
        if( $atts['username'] != '' ){
            ...
            return __("Please specify the twitter username.", 'newave_theme');
        }
    }
    else{

        return __("Please install and activate oAuth Twitter Feed for Developers plugin.", 'newave_theme');
    }
}
...
// Info Box
//////////////////////////////////////////////////////////////////
add_shortcode('newave_info_box', 'shortcode_newave_info_box');
function shortcode_newave_info_box($atts, $content = null) {
    ...
    $str .= '<div class="header-box"><i class="' . $atts['header_icon'] . ' fa-2x"></i></div>';                            
    $str .= '<div class="content-box"><h4>' . do_shortcode( $content ) . '</h4></div>';                            
    $str .= '<div class="footer-box"><p class="subtitle">' . $atts['footer_text'] . '</p></div>';                        
    $str .= '</div>';
