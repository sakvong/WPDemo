    ...
    if( $shortcode ){
        ...
        if( isset( $shortcode['attributes'] ) && $shortcode['attributes'] ){
            ...
            foreach( $attributes as $attribute => $attr_options ){
                ...
                if( $attr_options['type'] == "icon" ){
                    ...
                    echo '<input class="attr" type="text" data-attrname="'.$attribute.'" value="' . $attr_options['default'] . '" />';
                    echo '<a class="btn display-icons-list" id="btn-' . $attribute . '">' . __('Set Icon', 'newave_theme') . '</a>';
        ...
        if( isset( $shortcode['sub_items'] ) && $shortcode['sub_items'] ){
            ...
            foreach ( $shortcode['sub_items'] as $subitem_name => $subitem ) {
                ...
                echo '</div>';
                echo '</div>';
                echo '<a href="#" class="btn blue add-list-item">' . sprintf(__('Add %s', 'newave_theme' ), $subitem['name']) . '</a>' . '<a href="#" class="btn blue remove-list-item">'. sprintf(__('Remove %s', 'newave_theme' ), $subitem['name']) . '</a>';
            }
        ...
        if( isset( $shortcode['has_content'] ) && $shortcode['has_content'] ){
            ...
            <div id="shortcode-content">
                ...
                <div class="label"><label id="option-label" for="shortcode-content"><?php echo __( 'Content: ', 'newave_theme' ); ?> </label></div>
...
<a class="btn" id="add-shortcode"><?php _e('Add Shortcode', 'newave_theme'); ?></a>
