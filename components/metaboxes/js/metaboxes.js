...
/*----------------------------------------------------------------------------------*/
	...
	$('#newave_is_page_section').change(checkIsSection);
	
	function checkIsSection(){
		var format = $('#newave_is_page_section option:selected').attr('value');
		
		//only run on the posts page
		if( ( typeof format != 'undefined' ) && ( format == 'no' ) ){
			
			$('#post-body div[id^=newave_page_options_]').hide();
			$('#post-body #newave_page_options_external').stop(true,true).fadeIn(500);
